<?php 
    ...
    if(isset($_POST['dodajID'])){
        ...
        $selectText = $row['naziv_destinacije'];
         ...
         #tabela dani
         $sql = "SELECT * FROM dani WHERE id_destinacije = $selectedValue";
         $result = mysqli_query($conn, $sql);

    }
        if(isset($_POST['urediOpis'])){
            $selectedValue = $_SESSION['id_destinacije'];
    ...
    for($i=0; $i < $k; $i++){
        ...
        mysqli_query($conn, "UPDATE dani SET broj_dana =  '$br', opis_dana = '$op' WHERE id_destinacije = $selectedValue") 
        ...
        <form action="" method="POST">
            <div class="input-field col s5">
                <select class="icons" name="destination">
                    <option value="<?php echo ltrim($selectedValue) ?>"disabled selected><?php echo ltrim($selectText) ?></option>
                    
                    <?php if(mysqli_num_rows($res) > 0){
                        while ($plan = mysqli_fetch_array($res)){
                    ?>
                        <option value="<?php echo $plan['id']?>" name="id" data-icon="images/<?php echo $plan['slika'] ?>">
                            <?php echo $plan['naziv_destinacije']?>
                        </option>
                    <?php  
                        } 
                    }  
                    ...
                    <?php if($b > 0){ ?>
                        ...
                        <?php for ($x=1; $x <= (int)$b; $x++){ ?>
                            <?php if(mysqli_num_rows($result) > 0){
                                while ($dani = mysqli_fetch_array($result)){
                                ?>
                            <div class="row">
                                <div class="col s12">
                                    <div class="row">
                                        <div class="input-field col s3">
                                            ...
                                            <input value="<?php echo $dani['broj_dana'] ?>" placeholder="Day" type="text" name="dan[]">
                                            <label for="first_name">Day</label>
                                        </div>
                                        <div class="input-field col s9" style="margin-top: 0px; margin-bottom:0px; ">
                                            <i class="material-icons prefix">edit</i>
                                            <textarea  style="margin-bottom:0;" id="textarea1" name="opisDana[]" class="materialize-textarea">
                                                <?php echo urlencode($dani['opis_dana'], '') ?>
                                            </textarea>
                            ...
                            <div class="row">
                                <div class="input-field col s12 center-align">
                                    <button class="btn waves-effect waves-light"
                                        type="submit" name="urediOpis"  style="margin-bottom:15px;">
                                        EDIT
